...
var slnFile = "./Ocelot.Provider.Polly.sln";
...
var unitTestAssemblies = @"./test/Ocelot.Provider.Polly.UnitTests/Ocelot.Provider.Polly.UnitTests.csproj";
var minCodeCoverage = 0d;
var coverallsRepoToken = "coveralls-repo-token-ocelot";
var coverallsRepo = "https://coveralls.io/github/ThreeMammals/Ocelot.Provider.Polly";

// acceptance testing
var artifactsForAcceptanceTestsDir = artifactsDir + Directory("AcceptanceTests");
var acceptanceTestAssemblies = @"./test/Ocelot.Provider.Polly.AcceptanceTests/Ocelot.Provider.Polly.AcceptanceTests.csproj";

// integration testing
var artifactsForIntegrationTestsDir = artifactsDir + Directory("IntegrationTests");
var integrationTestAssemblies = @"./test/Ocelot.Provider.Polly.IntegrationTests/Ocelot.Provider.Polly.IntegrationTests.csproj";

// benchmark testing
var artifactsForBenchmarkTestsDir = artifactsDir + Directory("BenchmarkTests");
var benchmarkTestAssemblies = @"./test/Ocelot.Provider.Polly.Benchmarks";
...
// stable releases
var tagsUrl = "https://api.github.com/repos/threemammals/ocelot.Provider.Polly/releases/tags/";
	...
	.Does(() =>
	{
		if (IsRunningOnWindows())
		{
			...
			OpenCover(tool => 
				...
				.WithFilter("+[Ocelot.Provider.Polly*]*")
				.WithFilter("-[xunit*]*")
				.WithFilter("-[Ocelot.Provider.Polly*Tests]*")
			);
...
Task("CreatePackages")
	...
	.Does(() => 
	{
		...
		CopyFiles("./src/**/Ocelot.Provider.Polly.*.nupkg", packagesDir);

		//GenerateReleaseNotes(releaseNotesFile);

        System.IO.File.WriteAllLines(artifactsFile, new[]{
            "nuget:Ocelot.Provider.Polly." + buildVersion + ".nupkg",
